...
from diffusent.entities import Document, Dataset, EntityType
from diffusent.input_reader import JsonInputReader
import jinja2
...
class Evaluator:
    def __init__(self, dataset: Dataset, text_encoder: AutoTokenizer, logger, no_overlapping: bool, no_partial_overlapping: bool, no_duplicate: bool, predictions_path: str, examples_path: str, example_count: int, epoch: int, dataset_label: str, cls_threshold = 0, boundary_threshold = 0, entity_threshold = 0, save_prediction = False):
        ...
        self._convert_gt_rel(self._dataset)
    ...
    def eval_batch(self, outputs, batch = None):
        ...
        entity_left = pred_left.argmax(dim=-1)#[b,gt]
        entity_right = pred_right.argmax(dim=-1)#[b,gt]
        batch_entity_left_scores = pred_left.max(dim=-1)[0]#[b,gt]
        batch_entity_right_scores = pred_right.max(dim=-1)[0]#[b,gt]

        entity_prob = entity_logits.softmax(-1)#[b,gt,3]
        batch_entity_types = entity_prob.argmax(dim=-1)#[b,gt]
        ...
        for i in range(batch_size):
            ...
            sample_pred_entities = self._convert_pred_entities(valid_entity_types, valid_entity_spans, valid_entity_scores, valid_left_scores, valid_right_scores, valid_type_scores)
    ...
    def eval_batch_rel(self, outputs, word2span = None):
        entity_logits = outputs["pred_logits"]
        pred_left_a = outputs["pred_left_a"]
        pred_right_a = outputs["pred_right_a"]
        pred_left_o = outputs["pred_left_o"]
        pred_right_o = outputs["pred_right_o"]

        batch_size = entity_logits.shape[0] 
        entity_left_a = pred_left_a.argmax(dim=-1)#[b,gt]
        entity_right_a = pred_right_a.argmax(dim=-1)#[b,gt]
        entity_left_o = pred_left_o.argmax(dim=-1)#[b,gt]
        entity_right_o = pred_right_o.argmax(dim=-1)#[b,gt]
        batch_entity_left_a_scores = pred_left_a.max(dim=-1)[0]#[b,gt]
        batch_entity_right_a_scores = pred_right_a.max(dim=-1)[0]#[b,gt]
        batch_entity_left_o_scores = pred_left_o.max(dim=-1)[0]#[b,gt]
        batch_entity_right_o_scores = pred_right_o.max(dim=-1)[0]#[b,gt]
        ...
        batch_entity_spans = torch.stack([entity_left_a, entity_right_a, entity_left_o, entity_right_o], dim=-1)
        # batch_entity_mask = batch_entity_mask * (batch_entity_spans[:,:,0] <= batch_entity_spans[:,:,1])
        batch_entity_mask = batch_entity_mask * (batch_entity_spans[:,:,0] <= batch_entity_spans[:,:,1])*(batch_entity_spans[:,:,2] <= batch_entity_spans[:,:,3]) * ((batch_entity_left_a_scores > self._boundary_threshold) | (batch_entity_right_a_scores > self._boundary_threshold) | (batch_entity_left_o_scores > self._boundary_threshold) | (batch_entity_right_o_scores > self._boundary_threshold))

        batch_entity_mask = batch_entity_mask * ((batch_entity_left_a_scores + batch_entity_right_a_scores + batch_entity_left_o_scores + batch_entity_right_o_scores + batch_entity_scores) > self._entity_threshold)
        ...
        for i in range(batch_size):
            ...
            valid_left_a_scores = batch_entity_left_a_scores[i][entity_mask]
            valid_right_a_scores = batch_entity_right_a_scores[i][entity_mask]
            valid_left_o_scores = batch_entity_left_o_scores[i][entity_mask]
            valid_right_o_scores = batch_entity_right_o_scores[i][entity_mask]
            valid_entity_scores = valid_type_scores + valid_left_a_scores + valid_right_a_scores+valid_left_o_scores + valid_right_o_scores


            sample_pred_entities = self._convert_pred_entities_rel(valid_entity_types, valid_entity_spans, valid_entity_scores, valid_left_a_scores, valid_right_a_scores,valid_left_o_scores, valid_right_o_scores, valid_type_scores)
            sample_pred_entities = sorted(sample_pred_entities, key=lambda x:x[5], reverse=True)
            ...
            self._pred_entities.append(sample_pred_entities)
            self._word2span.append(word2span[i])
    ...
    def compute_scores(self):
        ...
        negative_sample=[]
        for preds in self._pred_entities:
            negative=[]
            print(preds)
            for pred in preds:
                if len(pred)>0:
                    negative.append(list(pred[:5])+[round(pred[5],2)])
            negative_sample.append(negative)
        dict={index: [value, "gt_num:"+str(len(self._gt_entities[index]))] for index, value in enumerate(negative_sample)}
        dict['epoch']=epoch
        with open('./DiffusionIE_11.2/loss/case.json', 'a') as f:
            json.dump(dict, f)
            f.write('\n')
    ...
    def _filter(self,entities):
        assert len(entities)==len(self._word2span)
        for i in range(len(entities)):
            entities[i] = [span for span in entities[i] if (self._word2span[i][span[0]]==self._word2span[i][span[1]-1] and self._word2span[i][span[2]]==self._word2span[i][span[3]-1])] 
            # for span in entities[i]:
            #     if (self._word2span[i][span[0]]==self._word2span[i][span[1]-1]) and (self._word2span[i][span[2]]==self._word2span[i][span[3]-1]):
            #         del span
        return entities
    
    def _filter1(self,gt,pred):
    ...
    def compute_scores_rel(self, epoch):
        self._log("Evaluation")
        ...
        # self._log("An entity is considered correct if the entity type and span is predicted correctly")
        self._log("")
        gt, pred = self._convert_by_setting_rel(self._gt_entities, self._pred_entities, include_entity_types=True)
        ner_eval = self._score_rel(gt, pred, print_results=True)
        self._log(str(ner_eval))


        # self._log("--- multi-word ---")
        # gt1= self._filter(self._gt_entities)
        # pred1=self._filter(self._pred_entities)
        # gt, pred = self._convert_by_setting_rel(gt1, pred1, include_entity_types=True)
        # ner_eval_multi = self._score_rel(gt, pred, print_results=True)
        # print(ner_eval_multi)

        self._log("--- multi-triplet ---")
        gt1, pred1= self._filter1(self._gt_entities, self._pred_entities)
        gt, pred = self._convert_by_setting_rel(gt1, pred1, include_entity_types=True)
        ner_eval_multi = self._score_rel(gt, pred, print_results=True)
        print(ner_eval_multi)
        ...
        # # dict['epoch']=epoch
        # if epoch-1==0:
        #     print(self._gt_entities)
        #     print("***result***")
        #     print(self._pred_entities)
        # else:
        #     print("***result***")
        #     print(self._pred_entities)

        # negative_sample=[]
        # for gts, preds in zip(self._gt_entities, self._pred_entities):
        #     negative=[]
        #     for pred in preds:
        #         is_negative=True
        #         for gt in gts:
        ...
        # dict={index: [value, "gt_num:"+str(len(self._gt_entities[index]))] for index, value in enumerate(negative_sample)}
        # dict['epoch']=epoch
        # with open('./DiffuSent/loss/negative_result_3.0.json', 'a') as f:
        #     json.dump(dict, f)
        #     f.write('\n')
    ...
    def _convert_gt(self, dataset):
        for example in dataset:
            gt_entities = example[2]
            gt_types=example[4]
            # if len(doc.encoding) > 512:
            #     continue
            ...
            sample_gt_entities = [(*entity,type) for entity,type in zip(gt_entities,gt_types)]
    ...
    def _convert_gt_rel(self, dataset):
        for example in dataset:
            gt_entities = example[3]
            gt_types=example[5]
            # if len(doc.encoding) > 512:
            #     continue
            # convert ground truth relations and entities for precision/recall/f1 evaluation
    ...
    def _convert_pred_entities(self, pred_types: torch.tensor, pred_spans: torch.tensor, pred_scores: torch.tensor,  left_scores, right_scores, type_scores):
        converted_preds = []
        ...
        for i in range(pred_types.shape[0]):
            ...
            converted_pred = (start, end, label_idx, entity_score)
    ...
    def _convert_pred_entities_rel(self, pred_types: torch.tensor, pred_spans: torch.tensor, pred_scores: torch.tensor,  left_a_scores, right_a_scores,left_o_scores, right_o_scores, type_scores):
        converted_preds = []
        ...
        for i in range(pred_types.shape[0]):
            ...
            start_a, end_a,start_o, end_o  = pred_spans[i].tolist()
            entity_score = pred_scores[i].item()
            cls_score = type_scores[i].item()
            left_a_score = left_a_scores[i].item()
            right_a_score = right_a_scores[i].item()
            left_o_score = left_o_scores[i].item()
            right_o_score = right_o_scores[i].item()

            converted_pred = (start_a, end_a,start_o, end_o, label_idx, entity_score)
            converted_preds.append(converted_pred)
            # decode_entity["entities"].append({"start": start, "end": end, "entity_type":entity_type.identifier, "cls_score": round(cls_score, 2), "left_score": round(left_score, 2), "right_score": round(right_score, 2), "entity_score": round(entity_score, 2)})
        # self._raw_preds.append(decode_entity)
        return converted_preds
    ...
    def _remove_duplicate(self, entities):
        ...
        for i, can_entity in enumerate(entities):
            ...
            for j, entity in enumerate(non_duplicate_entities):
                if can_entity[0] == entity[0] and can_entity[1] == entity[1] and can_entity[2] == entity[2] and can_entity[3] == entity[3]:
                    find = True
            if not find:
    ...
    def _convert_by_setting_rel(self, gt: List[List[Tuple]], pred: List[List[Tuple]],
                            include_entity_types: bool = True, include_score: bool = False):
        assert len(gt) == len(pred)
    ...
    def _score_rel(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = False, cls_metric = False):
        assert len(gt) == len(pred)
        # import pdb;pdb.set_trace()
        ...
        for (sample_gt, sample_pred) in zip(gt, pred):
            ...
            if cls_metric:
                union.update(sample_gt)
                loc_gt = list(map(lambda x:(x[0],x[1],x[2],x[3]), sample_gt))
                sample_loc_true_pred =  list(filter(lambda x:(x[0],x[1],x[2],x[3]) in  loc_gt, sample_pred))
                union.update(sample_loc_true_pred)
            else:
                union.update(sample_gt)
                union.update(sample_pred)
            ...
            for s in union:
                ...
                if s in sample_pred:
                    ...
                    pred_flat.append(-1)
        metrics = self._compute_metrics_rel(gt_flat, pred_flat, types, print_results)
        return metrics
    ...
    def _compute_metrics_rel(self, gt_all, pred_all, types, print_results: bool = False):
        labels = [t for t in types]
        per_type = prfs(gt_all, pred_all, labels=labels, average=None)
        micro = prfs(gt_all, pred_all, labels=labels, average='micro')[:-1]
        macro = prfs(gt_all, pred_all, labels=labels, average='macro')[:-1]
