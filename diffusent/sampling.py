...
def create_train_sample(doc, repeat_gt_entities = 100):
    ...
    token_count = len(doc.tokens) # tokens per sentence wo context   max:48
    context_size = len(encodings) # encoding per sentence with context  max:168

    context2token_masks = []
    for t in doc.tokens:
        context2token_masks.append(create_entity_mask(*t.span, context_size)) #每个句子中的token在encoding中的位置
    # Xinhua News Agency   ,    Cannberra     ,     August    31st
    #(1,3)  (4,4)  (5,5) (6,6)    (7,7)     (8,8)    (9,9)  (10,10)
